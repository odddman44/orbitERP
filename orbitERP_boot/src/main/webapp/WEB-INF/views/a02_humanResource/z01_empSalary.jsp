...
<script>
	$(document).ready(function() {
				...
				success : function(data) {
					...
					$.each(data, function(index, employee) {
						...
						empSelect.append($("<option>", {
							...
							text : employee.empno + ' [' + employee.ename + ']' // 옵션의 텍스트는 사원 이름
						}));
					});
				},
		...
		$('#base_salary, #allowance, #deduction').on('input', function() {
			var baseSalary = $('#base_salary').val();
			var allowance = $('#allowance').val();
			var deduction = $('#deduction').val();
			
			if (baseSalary && allowance && deduction) {
				baseSalary = parseInt(baseSalary.replace(/,/g, '')) || 0;
				allowance = parseInt(allowance.replace(/,/g, '')) || 0;
				deduction = parseInt(deduction.replace(/,/g, '')) || 0;

				var netPay = baseSalary + allowance - deduction;
				// 콤마 추가
				var formattedNetPay = addCommas(netPay);

				// net_pay 필드에 값을 설정합니다.
				$('#net_pay').val(formattedNetPay);
			}
	...
	function search() {
		...
		$.ajax({
			...
			success : function(data) {
				console.log(data)
				var tableBody = $("#empSalary tbody")
				$.each(data,
						function(index, salary) {
							var row = "<tr>";
							...
							row += "<td>" + salary.empno + "</td>";
							row += "<td>" + salary.base_salary.toLocaleString()
									+ "</td>";
							row += "<td>" + salary.allowance.toLocaleString()
									+ "</td>";
							row += "<td>" + salary.deduction.toLocaleString()
									+ "</td>";
							row += "<td>" + salary.net_pay.toLocaleString()
									+ "</td>";
							row += "<td>" + salary.dname + "</td>";
							row += "</tr>";
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
			...
			<div class="table-responsive">
				<table class="table table-bordered" id="empSalary">
					<thead>
						<tr>
					...
					<div class="form-group row">
						...
						<div class="col-md-4">
							...
							<label for="start_date">급여 지급일</label> <input type="date"
								class="form-control" id="payment_date" name="payement_dateStr"
								required />
						</div>
						<!-- 거래처명 표시 -->
						<div class="col-md-4">
						...
						<div class="col-md-4">
							...
							<label for="end_cname">근무 종료일자</label> <input type="date"
								class="form-control" id="end_date" name="end_cnameStr"
