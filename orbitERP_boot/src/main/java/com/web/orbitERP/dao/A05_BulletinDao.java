...
public interface A05_BulletinDao {
	...
	@Select("SELECT fname FROM bulletinFile WHERE NO = #{no}")
	List<String> getBulletinFile(int no);
	
	@Delete("DELETE FROM bulletinFile WHERE NO = #{no}")
	int deleteBulletinFile(@Param("no") int no);
	*/
}
